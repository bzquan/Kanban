...
namespace DragAndDropLib
{
    ...
    {
        private bool _isDraggingDisallowed;
        ...
        {
            _isDraggingDisallowed = false;
            _isMouseDown = false;
            _isDragging = false;
            _dragScrollWaitCounter = DRAG_WAIT_COUNTER_LIMIT;
            this.Loaded += new RoutedEventHandler(DraggableItemsControl_Loaded);
            Kanban.Util.EventAggregator<DisallowDragAndDropRequestArg>.Instance.Event += OnDisallowDragAndDropRequest;
            Kanban.Util.EventAggregator<AllowDragAndDropRequestArg>.Instance.Event += OnAllowDragAndDropRequest;
        }

        private void OnDisallowDragAndDropRequest(object sender, DisallowDragAndDropRequestArg arg)
        {
            _isDraggingDisallowed = true;
            ResetState();
        }

        private void OnAllowDragAndDropRequest(object sender, AllowDragAndDropRequestArg arg)
        {
            _isDraggingDisallowed = false;
            ResetState();
        }
        ...
        {
            if (_isDraggingDisallowed) return;
        ...
        private void HandleDragScrolling(ItemsControl itemsControl, DragEventArgs e)
        {
            ...
            {
                bool use_horizontal_insert_adorner = Helper.IsItemControlOrientationHorizontal(itemsControl);
                bool use_vertical_scroll = (scrollViewer.ComputedVerticalScrollBarVisibility == Visibility.Visible) &&
                                            use_horizontal_insert_adorner;
                bool use_horizontal_scroll = (scrollViewer.ComputedHorizontalScrollBarVisibility == Visibility.Visible) &&
                                            !use_horizontal_insert_adorner;

                if (use_vertical_scroll)
                {
                    HandleDragVerticalScrolling(scrollViewer, itemsControl, e);
                }
                else if (use_horizontal_scroll)
                {
                    HandleDragHorizontalScrolling(scrollViewer, itemsControl, e);
                }
            }
            else
            {
        ...
        private void HandleDragVerticalScrolling(ScrollViewer scrollViewer, ItemsControl itemsControl, DragEventArgs e)
        {
            if (_isDraggingDisallowed) return;

            bool? isMouseAtTop = Helper.IsMousePointerAtTop(itemsControl, e.GetPosition(itemsControl));
            if (!isMouseAtTop.HasValue) return;
        ...
        private void HandleDragHorizontalScrolling(ScrollViewer scrollViewer, ItemsControl itemsControl, DragEventArgs e)
        {
            if (_isDraggingDisallowed) return;

            bool? isMouseAtLeft = Helper.IsMousePointerAtLeft(itemsControl, e.GetPosition(itemsControl));
            if (!isMouseAtLeft.HasValue) return;

            if (_dragScrollWaitCounter == DRAG_WAIT_COUNTER_LIMIT)
            {
                _dragScrollWaitCounter = 0;
                if (isMouseAtLeft.Value)
                {
                    scrollViewer.ScrollToHorizontalOffset(scrollViewer.HorizontalOffset - 100.0); // T.B.D.: why 100 unit instead of one item?
                }
                else
                {
                    scrollViewer.ScrollToHorizontalOffset(scrollViewer.HorizontalOffset + 100.0);
                }
                e.Effects = DragDropEffects.Scroll;
            }
            else
